...
public class MutantService {
    ...
    public boolean analyzeDna(String[] dna) {
        ...
        boolean isMutant = isMutant(dna);         // Analizamos si es mutante

        Mutant dnaEntity = Mutant.builder()        // Guardamos el resultado en la base de datos
                .dna(dnaSequence)
                .isMutant(isMutant)
                .build();
    ...
    public static boolean isMutant(String[] dna) {    // Método que verifica si el ADN es mutante
        int n = dna.length;
        int sequenceCount = 0;
